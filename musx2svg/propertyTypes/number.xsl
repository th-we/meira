...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template match="def:number" mode="generic-default">
    <copy-of select="0"/>
  </xsl:template>
  <xsl:template match="def:number" mode="add-as-type-attribute">
    <xsl:attribute name="as">xs:double*</xsl:attribute>
  </xsl:template>
  
  <xsl:template match="def:number" mode="create-getter-templates">
    ...
    <template mode="get_{@name}" match="{g:matchPattern($elementNames,concat('[@',@name,']'))}">
      <copy-of select="@{@name} cast as xs:double"/>
    </template>
  </xsl:template>
